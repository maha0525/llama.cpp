...
//
// Sidecar file format:
//   [magic "MTMD" 4][version u32][mmproj_hash 32B][total_tokens u64][n_chunks u64]
...
constexpr uint32_t MTMD_SIDECAR_VERSION         = MTMD_INPUT_CHUNK_SERIALIZATION_VERSION;
constexpr uint32_t MTMD_SIDECAR_MIN_VERSION     = 2;
constexpr size_t   MTMD_SIDECAR_HASH_LEN        = 32;  // SHA-256
constexpr uint64_t MTMD_SIDECAR_MAX_CHUNK_BYTES = 256ULL * 1024 * 1024;
...
size_t server_tokens::save_mtmd_sidecar(const std::string & filepath,
    ...
    for (const auto & it : map_idx_to_media) {
        ...
        if (need == 0 || chunk_size > MTMD_SIDECAR_MAX_CHUNK_BYTES) {
            SRV_ERR("save_mtmd_sidecar: invalid chunk size %zu at idx %" PRIu64 "\n",
                    need, start_idx);
            return 0;
        }
...
bool server_tokens::load_mtmd_sidecar(const std::string & filepath,
    ...
    if (version < MTMD_SIDECAR_MIN_VERSION || version > MTMD_SIDECAR_VERSION) {
        SRV_ERR("load_mtmd_sidecar: unsupported version %u in %s (supported %u-%u)\n",
                version, filepath.c_str(), MTMD_SIDECAR_MIN_VERSION, MTMD_SIDECAR_VERSION);
        return false;
    }
    ...
    if (n_chunks > total_tokens) {
        SRV_ERR("load_mtmd_sidecar: invalid chunk count %" PRIu64 " in %s\n",
                n_chunks, filepath.c_str());
        return false;
    }

    // Read chunks
    decltype(map_idx_to_media) loaded_media;
    std::vector<bool> media_positions(tokens.size(), false);
    std::vector<uint8_t> chunk_buf;
    for (uint64_t i = 0; i < n_chunks; ++i) {
        ...
        size_t bytes_read = 0;
        mtmd_input_chunk * raw = mtmd_input_chunk_deserialize_version(
            chunk_buf.data(), chunk_buf.size(), &bytes_read, version);
        if (!raw) {
        ...
        if (actual_type != MTMD_INPUT_CHUNK_TYPE_IMAGE &&
            actual_type != MTMD_INPUT_CHUNK_TYPE_AUDIO) {
            SRV_ERR("load_mtmd_sidecar: non-media chunk at idx %" PRIu64 " in %s\n",
                    i, filepath.c_str());
            mtmd_input_chunk_free(raw);
            return false;
        }

        const size_t start = static_cast<size_t>(start_idx);
        const size_t n_tokens = mtmd_input_chunk_get_n_tokens(raw);
        if (start_idx > std::numeric_limits<size_t>::max() ||
            start >= tokens.size() || n_tokens == 0 || n_tokens > tokens.size() - start) {
            SRV_ERR("load_mtmd_sidecar: invalid token range at idx %" PRIu64 " in %s\n",
                    i, filepath.c_str());
            mtmd_input_chunk_free(raw);
            return false;
        }
        for (size_t pos = start; pos < start + n_tokens; ++pos) {
            if (tokens[pos] != LLAMA_TOKEN_NULL || media_positions[pos]) {
                SRV_ERR("load_mtmd_sidecar: media range does not match token placeholders at idx %"
                        PRIu64 " in %s\n", i, filepath.c_str());
                mtmd_input_chunk_free(raw);
                return false;
            }
            media_positions[pos] = true;
        }
        loaded_media.emplace(start, mtmd::input_chunk_ptr(raw));
    }

    for (size_t pos = 0; pos < tokens.size(); ++pos) {
        if ((tokens[pos] == LLAMA_TOKEN_NULL) != media_positions[pos]) {
            SRV_ERR("load_mtmd_sidecar: incomplete media coverage at token %zu in %s\n",
                    pos, filepath.c_str());
            return false;
        }
    }
    ...
    if (in.peek() != std::char_traits<char>::eof()) {
        SRV_ERR("load_mtmd_sidecar: trailing data in %s\n", filepath.c_str());
        return false;
    }

    map_idx_to_media = std::move(loaded_media);
    return true;
}
