...
bool batch_f32_serialized_size(const clip_image_f32_batch & batch, size_t & result, uint32_t version) {
    ...
    for (const auto & entry_ptr : batch.entries) {
        if (!entry_ptr) {
            return false;
        }
...
size_t mtmd_input_chunk_serialized_size_version(const mtmd_input_chunk * chunk, uint32_t version) {
    if (!chunk || version < 2 || version > MTMD_INPUT_CHUNK_SERIALIZATION_VERSION) return 0;
...
size_t mtmd_input_chunk_serialize_version(
    ...
    if (!chunk || !buf || version < 2 || version > MTMD_INPUT_CHUNK_SERIALIZATION_VERSION) return 0;
...
mtmd_input_chunk * mtmd_input_chunk_deserialize_version(
    ...
    if (!buf || version < 2 || version > MTMD_INPUT_CHUNK_SERIALIZATION_VERSION) {
        return nullptr;
    }
...
mtmd_input_chunks * mtmd_test_create_input_chunks() {
    ...
    image_tokens->nx = 4;
    image_tokens->ny = 4;
    image_tokens->batch_f32.entries.emplace_back(new clip_image_f32);
    ...
    audio_tokens->batch_f32.entries.emplace_back(new clip_image_f32);
